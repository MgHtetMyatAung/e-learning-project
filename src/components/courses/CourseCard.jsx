...
export default function CourseCard({course}){
    return (
        <div className="tablet:p-12 p-5 rounded-[12px] bg-white-1 space-y-5 shadow">
        {/* title and content  */}
        <div className="flex gap-8 tablet:justify-center tablet:items-center tablet:flex-row flex-col">
          <div className="flex-1 space-y-2">
            <h6 className="font-[600] text-grey-7 capitalize">
              {course.title}
            </h6>
          ...
          <div className="text-left">
            <a className="text-grey-7 justify-start tablet:font-normal font-semibold">View Course</a>
          </div>
        </div>
        {/* cousre cover images  */}
        <div className="grid grid-cols-3 gap-5">
            {Array.from({length:3},(_,i)=>(
                <div key={i} className="">
                <img src={`/src/assets/images/courses/${course.img_name}${i+1}.png`} alt="img1" />
              </div>
            ))}
        ...
        <div className="flex justify-between px-2 tablet:flex-row flex-col">
          <div className="space-x-7 text-grey-4">
            <span>{course.posted_date} week{course.posted_date > 1 ? 's' : ''}</span>
