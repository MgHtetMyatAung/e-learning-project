export { default as HomePage } from "./home/Home.page";
export { default as AboutUsPage } from "./about/AboutUs.page";
export { default as CoursesPage } from "./courses/Courses.page";
export { default as CourseDetail } from "./courses/CourseDetail.page";
export { default as LoginPage } from "./login/Login.page";
export { default as SignUpPage } from "./signUP/SignUp.page";
export { default as PricePage } from "./price/Price.page";
